...
- (IBAction)onSubmitBtn:(id)sender {
      NSString *result = @"Please enter description.";
if([_descritptionTextView.text isEqualToString:@"Write here."])
{
...
-(void)customInit
{
  ...
  //  _feedImageView.layer.borderColor = [UIColor whiteColor];
...
-(void)sendData:(NSMutableArray*)array
{
    ...
    for(int i = 0; i<array.count; i++)
        {
            self.likeList.text = [self.likeList.text stringByAppendingString:[array objectAtIndex:i]];
            self.likeList.text = [self.likeList.text stringByAppendingString:@"\n"];
